...
class RegisterEmailView(AnonymousRequiredMixin,View):
    ...
    def post(self,request,*args, **kwargs):
        ...
        cache.set(key = key, value= value,timeout=EMAIL_EXPIRY_DURATION) # TTL = 15 minutes
...
class CompleteRegistrationView(View):
    ...
    def get(self,request, *args, **kwargs):
        """
        GET request means user clicked the link within 15 minutes, so reset the TTL to 15 minutes 
        """
        ...
        if not val['inc_TTL']:
            val['inc_TTL'] = True
            cache.set(key = key,value= val,timeout=EMAIL_EXPIRY_DURATION)
...
class UpdateProfile(LoginRequiredMixin,View):
    ...
    for 15 minutes. When the link is clicked, we will identify the user from user_id<base_64>. If the link is tampered,
    ...
    15 minutes the user_id will get unsign and coverted to base64. Corresponding to that user_id, new_email stored in redis
    ...
    def post(self, request, *args, **kwargs):
        ...
        cache.set(key=key, value={"new_email": new_email}, timeout=EMAIL_EXPIRY_DURATION)
